	...
	if target_velocity.y > 0.0:
		...
		if steep > 0.0:
			velocity.y = move_toward(
				velocity.y,
				target_velocity.y * steep,
				climb_acceleration * delta * 0.75
			)
...
func _handle_landing_check() -> void:
	if is_on_floor() and not was_on_floor:
		...
		if impact_speed >= min_fall_speed_for_shake:
			add_trauma(
				clamp(
					impact_speed * landing_trauma_per_speed,
					0.0,
					1.0
				)
			)
			...
			jump_kick_offset = (
				-jump_kick_amount *
				clamp(impact_speed / 10.0, 0.5, 1.5)
			)
